...
public class QuitAcceptMenu : MonoBehaviour {
    ...
    void Menu_Quit()
    {
        ...
        GUI.Label(new Rect(0, 0 * 80, Screen.width, 64), "Do you really want to quit?");
        if (GUI.Button(new Rect(0, 1 * 80, Screen.width, 64), "Yes")) { Menu_Quit_Yes(); }
        if (GUI.Button(new Rect(0, 2 * 80, Screen.width, 64), "No")) { Menu_Quit_No(); }
